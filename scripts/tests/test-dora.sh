...
WS="${TMP}/ws"
mkdir -p "${WS}/repos" "${WS}/knowledge/incidentes" "${WS}/scripts"
...
cat > "${WS}/knowledge/incidentes/INC-0001.md" <<EOF
...
GEN=$(sed -n '/<!-- DORA:BEGIN -->/,/<!-- DORA:END -->/p' "${WS}/knowledge/uso.md")
assert_contains "sello GENERADO presente"            "[GENERADO"        "$GEN"
assert_contains "frecuencia: 2 despliegues (repo-a)" "2 despliegues"    "$GEN"
assert_contains "lead time mediano 2d"               "2d"               "$GEN"
assert_contains "CFR 50% (1 incidente / 2 deploys)"  "50%"              "$GEN"
assert_contains "MTTR mediano 45m"                   "45m"              "$GEN"
...
assert_contains "tabla manual estimado-vs-real intacta" "$MANUAL_ANTES" "$(cat "${WS}/knowledge/uso.md")"
FUERA=$(sed '/<!-- DORA:BEGIN -->/,/<!-- DORA:END -->/d' "${WS}/knowledge/uso.md")
assert_contains "titulo manual intacto fuera del bloque" "## Estimado vs. real" "$FUERA"

echo "== idempotencia: segunda corrida identica =="
COPIA1=$(cat "${WS}/knowledge/uso.md")
./scripts/dora.sh --root "${WS}" >/dev/null 2>&1
assert_eq "recorrer sin cambios no altera uso.md" "$COPIA1" "$(cat "${WS}/knowledge/uso.md")"

echo "== --check: limpio=0, drift=1 =="
./scripts/dora.sh --root "${WS}" --check >/dev/null 2>&1
assert_eq "check en sincronia devuelve 0" "0" "$?"
sed -i.bak 's/45m/99m/' "${WS}/knowledge/uso.md" && rm -f "${WS}/knowledge/uso.md.bak"
./scripts/dora.sh --root "${WS}" --check >/dev/null 2>&1
...
cat > "${WS}/knowledge/incidentes/INC-0002.md" <<EOF
# Postmortem: degradación demo (INC-0002)
- **Fecha / Duración / Severidad:** $(date_ago 2) / 95m / SEV3
- **MTTR:** 95m
EOF
./scripts/dora.sh --root "${WS}" >/dev/null 2>&1
GEN2=$(sed -n '/<!-- DORA:BEGIN -->/,/<!-- DORA:END -->/p' "${WS}/knowledge/uso.md")
assert_contains "CFR sube a 100% (2 incidentes / 2 deploys)" "100%" "$GEN2"
assert_contains "MTTR mediano recalculado (70m)"             "70m"  "$GEN2"

echo "== CA1.3: workspace sin repos clonados declara sin datos =="
WS2="${TMP}/ws2"
mkdir -p "${WS2}/repos" "${WS2}/knowledge/incidentes" "${WS2}/scripts"
cp scripts/repo-lib.sh "${WS2}/scripts/"
cp "${WS}/repos.yaml" "${WS2}/repos.yaml"
sed -n '1,7p' "${WS}/knowledge/uso.md" > /dev/null # noop
cat > "${WS2}/knowledge/uso.md" <<'EOF'
# Uso
<!-- DORA:BEGIN -->
<!-- DORA:END -->
EOF
./scripts/dora.sh --root "${WS2}" >/dev/null 2>&1
GEN3=$(sed -n '/<!-- DORA:BEGIN -->/,/<!-- DORA:END -->/p' "${WS2}/knowledge/uso.md")
assert_contains "declara sin datos"            "sin datos"  "$GEN3"
assert_contains "explica la causa (no clonado)" "clonado"   "$GEN3"
