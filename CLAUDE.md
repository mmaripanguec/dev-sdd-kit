...
The current system (name, repos, providers) is declared in `repos.yaml`.
...
- Architecture standard (arc42 + C4 + 42010 + TOGAF/DDD/SRE, AI-consumable):
  `docs/architecture-documentation-standard.md` — defines the sections, views,
  ADR format, traceability and metadata the architecture doc must carry.
- Architecture (as-is, arc42 + C4): `knowledge/architecture/<system>.md` (+`.html`),
  GENERATED by `scripts/generate-architecture.sh` after indexing (called from
  `generate-as-is.sh`). Curated analysis lives in
  `knowledge/architecture/<system>.narrative.md`; derived data is injected.
  **Docs-first rule:** answer from this document and the context packs; do NOT
  re-index or re-read code for what is already documented. Consult the code /
  graph ONLY for undocumented gaps the human cannot answer — then update the
  narrative, regenerate, and add an assertion. Agent trigger: the
  `<prefix>-architecture` skill.
