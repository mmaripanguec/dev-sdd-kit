...
1. Read the spec, knowledge/reglas-negocio.md and knowledge/incidentes/ (the
...
2. Business rules: extract every rule implicit in the stories and make it
   ...
   knowledge/reglas-negocio.md; if it contradicts an existing one, ESCALATE.
3. Dependencies: map which domains, external systems, teams and data the
   ...
   - YES → verify that its pack exists and is current
     (scripts/frescura.sh, scripts/afirmaciones.sh); stale → request /repo-map.
